...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("Please provide a positive integer number of shares.")
        ...
        flash(f"Successfull transaction : Bought {shares} shares  of {symbol} for {usd(total_cost)}!")
        return redirect("/")
...
def register():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            return apology("Please provide a valid username.", 400)

        # check if pw entered
        elif not request.form.get("password"):
            return apology("Please provide a valid password.", 400)

        # check if confirmation of pw was entered
        elif not request.form.get("confirmation"):
            return apology("Must confirm password.", 400)

        # confirm entered pw
        elif request.form.get("password") != request.form.get("confirmation"):
            return apology("Passwords do not macht, please try again.", 400)
        ...
        if len(rows) != 0:
            return apology("Username already exists. Please use a different username.", 400)

        # when checks complete enter user into db
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares :
                    ...
                    flash(f"Transaction successful : Sold {shares} shares of {symbol} for {usd(total_sale)}!")
                    return redirect("/")
